...
OUTPUT_FOLDER_NAME = "features"


# parse arguments and initialize variables
parser = argparse.ArgumentParser()
parser.add_argument("-j", "--jpg", default="no", help="Save the images as .jpg files")
...
parser.add_argument("-e", "--expressions", default="0,1,2,3,4,5,6", help="Choose the facial expression you want to use: 0 = Angry, 1 = Disgust, 2 = Fear, 3 = Happy, 4 = Sad, 5 = Surprise, 6 = Neutral")
...
for category in data['Usage'].unique():
    print( "Converting the set: " + category)
    # create folder
    ...
    for i in range(len(samples)):
        try:
            if labels[i] in SELECTED_LABELS and nb_images_per_label[get_new_label(labels[i])] < IMAGES_PER_LABEL:
                image = np.fromstring(samples[i], dtype='uint8', sep=" ").reshape((image_height, image_width))
                images.append(image)
                if SAVE_IMAGES:
            ...
            print( "Error in the image: " + str(i) + " - " + str(e))
