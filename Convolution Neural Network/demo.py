...
def demo(modelPath, showBox=False):
  ...
  while True:
    ...
    if result is not None:
      for index, emotion in enumerate(EMOTIONS):
        cv2.putText(frame, emotion, (10, index * 20 + 290), cv2.FONT_HERSHEY_TRIPLEX, 0.40, (0, 0, 0), 1)
        cv2.rectangle(frame, (80, index * 20 + 280), (80 + int(result[0][index] * 100), (index + 1) * 20 + 274),
                      (0, 0, 255), -1)
        emoji_face = feelings_faces[np.argmax(result[0])]

      for c in range(0, 3):
        frame[10:130, 500:620, c] = emoji_face[:, :, c] * (emoji_face[:, :, 3] / 255.0) + frame[200:320, 10:130, c] * (1.0 - emoji_face[:, :, 3] / 255.0)
